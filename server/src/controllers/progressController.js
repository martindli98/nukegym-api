...
export const insertProgress = async (req, res) => {
  ...
  try {
    ...
    // Filtrar solo ejercicios con peso válido (mayor a 0)
    const progresosValidos = progresos.filter((p) => p.peso && p.peso > 0);

    if (progresosValidos.length === 0) {
      return res
        .status(400)
        .json({
          success: false,
          message: "Debes ingresar al menos un peso válido.",
        });
    }
    ...
    // 2️⃣ Insertar detalles solo para ejercicios con peso válido
    const insertPromises = progresosValidos.map((p) =>
      connection.query(
        `INSERT INTO Progreso_detalle (id_progreso, id_ejercicio, peso, repeticiones)
         VALUES (?, ?, ?, ?)`,
        [id_progreso, p.id_ejercicio, p.peso, p.repeticiones || null]
      )
    );
