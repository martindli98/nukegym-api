...
function Membership() {
  ...
  if (error) {
    membershipCardRender = (
      <MembershipCard title={error} img={fotoBaja} descripcion="Error pa" />
    );
  } else if (membershipStatus === null) {
    membershipCardRender = (
      <MembershipCard title="Cargando estado de la membresía..." />
    );
    ...
    membershipCardRender = (
      <MembershipCard
        ...
        img={fotoActiva}
        descripcion="Disfruta antes q se termine"
      />
    );
  } else if (membership?.data?.estado === "baja") {
    membershipCardRender = (
      <MembershipCard title="Tu membresía está dada de baja" img={fotoBaja} />
    );
  } else {
    membershipCardRender = (
      <div
  ...
  if (error) {
    infoCardRender = <InfoCard estado={error} inicio="--" fin="--" tipo="--" />;
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {plansList.map((plan, idx) => (
                <MembershipType
                  key={plan.id}
                  title={plan.nombre}
                  img={[PlanBasico, PlanIntermedio, PlanPremium][idx]}
                  descripcion={plan.descripcion}
                  color={["purple", "blue", "yellow"][idx]}
                  precio={plan.precio}
                />
              ))}
            </div>
