...
const Feedback = () => {
    ...
    const getComments = async (req, res)=>{
            ...
            try {
              ...
              if (response.data.success) {
               ...
               setError(response.data.message || "No se encontraron los comentarios.");
              }
            } catch (err) {
             console.error(err);
              setError(err.response?.data?.message || "Error al consultar.");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Algo salió mal. Por favor intenta de nuevo."
      );
      ...
      <tbody>
        {comments.map((c, index) => (
          ...
          >
            ...
            <td className="p-2 border-b">{new Date(c.fecha).toLocaleString()}</td>
          </tr>
        ))}
      </tbody>
    </table>
  ...
  >
    {rol === 2 ? (
      <div className="max-w-lg w-full text-center p-6 bg-white rounded-lg shadow-md">
        ...
        {submitted && (
          <p className="text-green-600 font-bold mb-4">¡Gracias por tu comentario!</p>
        )}
