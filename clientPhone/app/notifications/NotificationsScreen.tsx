...
export default function NotificationsScreen() {
  ...
  const isClientOrTrainer =
    user?.tipo_rol === "cliente" || user?.tipo_rol === "entrenador";
  ...
  const handleSubmit = async () => {
    ...
    const dataToSend = {
      ...
      fecha_envio:
        formData.fecha_envio && formData.fecha_envio.trim()
          ? new Date(formData.fecha_envio).toISOString()
        ...
        <View style={styles.headerLeft}>
          ...
          <View style={styles.headerContent}>
            <Text style={styles.title}>
              {isAdmin ? "Panel de Notificaciones" : "Notificaciones"}
            </Text>
            {isAdmin && <Text style={styles.adminBadge}>ADMIN</Text>}
          </View>
        </View>

        <Ionicons name="notifications" size={22} style={{ color: "#f97316"}}  />
      </View>
